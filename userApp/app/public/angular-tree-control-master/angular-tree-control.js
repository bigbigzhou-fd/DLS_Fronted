        ...
        .directive( 'treecontrol', ['$compile', function( $compile ) {
            ...
            return {
                ...
                controller: ['$scope', '$templateCache', '$interpolate', 'treeConfig', function( $scope, $templateCache, $interpolate, treeConfig ) {
                    ...
                    ensureDefault($scope.options, "nodeChildren", "child");
                    ...
                    $scope.selectNodeHead = function(node,$event) {
                        ...
                        var len = $(".tree-label").length;
                        for(var i=0; i<len; i++){
                            $($(".tree-label")[i]).removeClass("selectedNode")
                        }
                        if($event){
                    ...
                    if(!template) {
                        template =
                            ...
                            'set-node-to-data>' +
                            '<i class="tree-branch-head" ng-class="iBranchClass()" ng-click="selectNodeHead(node)"><img src="public/images/fold-close.png" class="dls-image-f1"/></i>' +
                            '<i class="tree-leaf-head {{options.iLeafClass}}"><img src="public/images/bookmark.png" class="dls-image-f2"/></i>' +
                            '<div class="tree-label {{options.labelClass}}" ng-class="[selectedClass(), unselectableClass()]" ng-click="selectNodeHead(node,$event)" tree-transclude></div>' +
                            '<span ng-if="node.child.length == 0" ng-show="options.addBtn" class="tree-li-add-btn" ng-click="options.btnClick(node,$event)">+</span>' +
                            '<span ng-if="options.admin && node.isParent && !node.child[0].isParent" class="tree-li-add" ng-click="options.addTag(node,$event)">+</span>' +
                            '<span ng-if="options.admin" class="glyphicon glyphicon-pencil tree-li-edit" ng-click="options.editTag(node,$event)"></span>' +
                            '<span ng-if="options.admin" class="glyphicon glyphicon-trash tree-li-remove" ng-click="options.removeTag(node,$event)"></span>' +
                    ...
                    return function ( scope, element, attrs, treemodelCntr ) {
                        ...
                        scope.$watch("treeModel", function updateNodeOnRootScope(newValue) {
                            if (angular.isArray(newValue)) {
                                if (angular.isDefined(scope.node) && scope.node && scope.node.hasOwnProperty(scope.options.nodeChildren) && angular.equals(scope.node[scope.options.nodeChildren], newValue))
                                    return;
                                scope.node = {};
