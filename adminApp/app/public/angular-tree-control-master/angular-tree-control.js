                ...
                controller: ['$scope', '$templateCache', '$interpolate', 'treeConfig', function( $scope, $templateCache, $interpolate, treeConfig ) {
                    ...
                    if(!template) {
                        template =
                            ...
                            '<span ng-if="!node.child || node.child.length == 0" ng-show="options.addBtn" class="tree-li-add-btn" ng-click="options.btnClick(node,$event)">+</span>' +
                            '<span ng-show="options.adminEdit" class="tree-li-add" title="添加标签" ng-click="options.addTag(node,$event)">+</span>' +
                            '<span ng-show="options.adminEdit" class="glyphicon glyphicon-pencil tree-li-edit" title="rename" ng-click="options.editTag(node,$event)"></span>' +
                            '<span ng-show="options.adminEdit" class="glyphicon glyphicon-trash tree-li-remove" title="remove" ng-click="options.removeTag(node,$event)"></span>' +
